(module syntax-analyser racket
  (provide read-while-program)
...
(test (skip-whitespace (list #\ #\ )) empty)
(test (skip-whitespace (list #\a #\b)) (list #\a #\b))
...
(define (accept in out)
  (let ((skipped-in (skip-whitespace in)))
    (begin (vector-set! out 0 skipped-in)
           (equal? skipped-in empty))))

(display "accept")
(test (let ((out (make-vector 1))) (begin (accept empty out) out)) (vector empty))
(test (let ((out (make-vector 1))) (begin (accept (string->list "  ") out) out)) (vector empty))
(test (let ((out (make-vector 1))) 
        (begin (accept (string->list "  ab") out) out)) 
      (vector(string->list "ab") ))
(test (accept empty (make-vector 1)) #t)
...
(display "list-head")
(test (list-head (list #\a #\b) 0) empty)
...
(define (split-upper-case in)
  (cond 
    ((equal? in empty) (cons empty empty))
...
(test (split-upper-case (string->list "ABC")) (cons (string->list "ABC") empty))
(test (split-upper-case (string->list "def")) (cons empty (string->list "def")))
...
(test (split-lower-case (string->list "abc")) (cons (string->list "abc") empty))
(test (split-lower-case (string->list "ABCdef")) (cons empty (string->list "ABCdef")))
...
(test (split-numeric (string->list "123")) (cons (string->list "123") empty))
(test (split-numeric (string->list "def")) (cons empty (string->list "def")))
  ...
  (let* ((skipped-in (skip-whitespace in))
    ...
    (cond
      ((member (list->string (car splitted-in-lower-case)) (list "while" "for" "if" "cons" "car" "cdr" "nil" "eq" "nop")) 
       (begin (vector-set! out 0 skipped-in)
               #f))
      ((equal? (car splitted-in-lower-case) empty)
        (begin (vector-set! out 0 skipped-in)
               #f))
      (else
...
(define (read-variable variable in out)
  (let* ((skipped-in (skip-whitespace in))
    ...
    (if (equal? (car splitted-in-upper-case) empty)
        (begin (vector-set! out 0 skipped-in)
               #f)
             ...
             (begin (vector-set! as 0 
                                 (lambda (x) 
                                   (cons (IF (vector-ref as-exp 0) 
                                             ((vector-ref as-com-then 0) empty)
                                             ((vector-ref as-com-else 0) empty)) 
                                         x)))
                    #t)))))
