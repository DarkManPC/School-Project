(module cal racket
  (provide pair fst snd
           PROGR PROGR? PROGR->in PROGR->body PROGR->out 
           WHILE? FOR? SET? IF? NOP? 
           COMM? WHILE WHILE->cond WHILE->body
           FOR FOR->count FOR->body
           IF IF->cond IF->then IF->else 
           SET SET->var SET->expr 
           NOP
           EXPR? VAR? CST? EQ? CONS? NIL? HD? TL?     
           VAR VAR->name 
           CST CST->name 
           CONS CONS->arg1 CONS->arg2 
           HD HD->arg 
           TL TL->arg 
           EQ EQ->arg1 EQ->arg2 
           NIL
...
;; définition d'un type pair et de ses fonctions d'accès fst et snd
